...
### Install GrokSearch skills

The release packages also ship the repo-local GrokSearch skills for agent
tool-use guidance. Install the latest released skills into the current
directory:
...
Install them into an agent skill directory:

```bash
# Codex: ${CODEX_HOME:-$HOME/.codex}/skills
curl -fsSL https://raw.githubusercontent.com/MosRat/GrokSearch-rs/main/scripts/install-skills.sh | bash -s -- --target codex

# Claude Code / cc: ${CLAUDE_HOME:-$HOME/.claude}/skills
curl -fsSL https://raw.githubusercontent.com/MosRat/GrokSearch-rs/main/scripts/install-skills.sh | bash -s -- --target cc
```

Use `--version X.Y.Z` to pin to a release, `--version main` to install from
the current branch archive, or `--dest <dir>` for a custom skills directory.
The script installs the five `grok-search-*` skills and replaces same-named
skill directories by default; pass `--no-overwrite` to skip existing copies.
...
To keep the HTTP MCP endpoint running in the background, install the current
binary as a current-user service with `grok-search-rs mcp-service install`.
This uses Windows Scheduled Tasks, Linux `systemd --user`, or macOS
LaunchAgent. The installer copies the current binary to a stable user bin
directory and updates that copy when the current version is newer; on Linux it
also enables systemd linger when possible so the service can survive logout.
It does not change the default stdio snippets generated by `init`.
...
| `grok-search-rs mcp-service install|status|start|stop|uninstall ...` | Manage the optional current-user background HTTP MCP service for the current binary. |

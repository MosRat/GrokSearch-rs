...
//! server. Mirrors the Tavily rotation tests without adding dev dependencies.
...
const OK_BODY: &str = r#"{"data":[{"url":"https://example.com","title":"ok"}]}"#;
...
#[tokio::test]
async fn key_scoped_statuses_rotate_like_rate_limit() {
    for status in [401, 403, 432, 433] {
        let (base, auth_log) =
            spawn_mock_server(vec![(status, r#"{"error":"key scoped"}"#), (200, OK_BODY)]);
        let provider = provider_for(&base, "key-a,key-b");

        provider
            .search("rust", 3)
            .await
            .unwrap_or_else(|err| panic!("second key should succeed after {status}: {err}"));

        assert_eq!(
            auth_log.lock().expect("auth log lock").len(),
            2,
            "HTTP {status} should rotate to a second key"
        );
    }
}
...
async fn successive_requests_round_robin_across_keys() {
    ...
    for _ in 0..2 {
        provider.search("rust", 3).await.expect("mock returns 200");
    }

...
pub fn academic_pdf_cache_path() -> Option<PathBuf> {
...
pub fn academic_pdf_cache_path_for<I, K, V>(env_vars: I) -> Option<PathBuf>
where
    I: IntoIterator<Item = (K, V)>,
    K: Into<String>,
    V: Into<String>,
{
    let env: HashMap<String, String> = env_vars
        ...
        .collect();
    if let Some(explicit) = env
        .get("GROK_SEARCH_ACADEMIC_PDF_CACHE_PATH")
        .filter(|v| !v.is_empty())
    {
        return Some(PathBuf::from(explicit));
    }
